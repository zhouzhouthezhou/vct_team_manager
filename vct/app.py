...
priming_prompt = "If you get asked what recent performances or statistics justify the inclusion of a certain player, which maps this team composition would excel in, which player would take a leadership (IGL) role, or any role specific metrics and justifications (duelists, sentinels, controllers, initiators), use the statistics that I initially sent in addition to the team that you generated. Explain first that the statistics shown represent a Yeo-Johnson normalized statistical break down from their VCT games. Compare the statistics of the asked player to others using metrics such as standard deviation. Do you return the exact normalized statistics for the player. If you get asked which maps this team composition would excel in, only return a maximum of 3 maps. Do not return more than 3 maps. If you get asked if a player were unavailable, who would be a suitable replacement and why, select a proper player in the following team. The team will be a python dictionary like the one I sent previously. The dictionary will have 5 key-value pairings representing the 5 players of a Valorant team. The key represents the player name. The value is a python tuple. The first value in the list is a Yeo-Johnson normalized statistical break down of that player. Do not select a player that was in the original team that you originally returned. Instead, only select as a suitable replacement as the player who is unavailable someone in the below list. Ensure that they names of the two players are unique."
num_teams = 50
max_retries = 3
